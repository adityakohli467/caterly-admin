...
export default function OptionsPage() {
  ...
  const [optionValues, setOptionValues] = useState<OptionValue[]>([{ name: "", sort_order: 1, standard_price: 0, wholesale_price: 0 }])
  ...
  const { data: optionsData, isLoading, error: optionsError } = useQuery({
    queryKey: ["options", searchQuery, currentPage],
    queryFn: async () => {
  ...
  const handleEditOption = (option: Option) => {
    ...
    })) : [{ name: "", sort_order: 1, standard_price: 0, wholesale_price: 0 }])
  ...
  const addValueField = () => {
    setOptionValues([...optionValues, { name: "", sort_order: optionValues.length + 1, standard_price: 0, wholesale_price: 0 }])
  }
  ...
  const resetForm = () => {
    ...
    setOptionValues([{ name: "", sort_order: 1, standard_price: 0, wholesale_price: 0 }])
    setSelectedOption(null)
  }
                    ...
                    <tr key={`${option.option_id}-${valueIndex}`} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                      {valueIndex === 0 ? (
                        <>
                          ...
                          <td className="px-4 py-3 text-sm text-gray-700" rowSpan={values.length} style={{ fontFamily: 'Albert Sans' }}>
                            Radio Buttons
                          </td>
                        </>
                      ) : null}
